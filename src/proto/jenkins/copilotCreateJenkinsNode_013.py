...
def create_ssh_credential(jenkins_url, user, token, cred_id, ssh_user, private_key):
    """Create SSH credential in Jenkins via REST API (requires SSH Credentials plugin)."""
...
def main():
    ...
    parser.add_argument("--password", required=True, help="API token or password")
    ...
    if args.credential_id:
        if args.use_existing_key:
            ...
            # Requires SSH Credentials plugin
