...
Prototype - don't know if this works, did not exercise it.

✅ Usage Examples
1️⃣ Use an existing SSH key
python create_node.py \
  --url http://localhost:8080 \
  --user admin \
  --token 110f2026ab995280b43539eb3dd51b2be2 \
  --node-name agent-linux-1 \
  --host 192.168.1.100 \
  --ssh-user jenkins \
  --credential-id agent-ssh-key \
  --use-existing-key ~/.ssh/id_ed25519
...
python create_node.py \
  ...
  --user admin \
  --token 110f2026ab995280b43539eb3dd51b2be2 \
  --node-name agent-linux-2 \
  --host 192.168.1.101 \
  --ssh-user jenkins \
  --credential-id agent-ssh-key2
...
# ------------------------
# Jenkins SSH credential via REST with CSRF
# ------------------------
def create_ssh_credential(jenkins_url, user, token, cred_id, ssh_user, private_key):
    # CSRF handling
    crumb_resp = requests.get(f"{jenkins_url}/crumbIssuer/api/json", auth=(user, token))
    if crumb_resp.status_code == 404:
        crumb_header = {}  # CSRF not enabled
    else:
        crumb_data = crumb_resp.json()
        crumb_header = {crumb_data['crumbRequestField']: crumb_data['crumb']}

    # Check if credential exists
    check_url = f"{jenkins_url}/credentials/store/system/domain/_/credential/{cred_id}/api/json"
    resp = requests.get(check_url, auth=(user, token))
    ...
    r = requests.post(url, auth=(user, token),
                      headers={**crumb_header, "Content-Type": "application/json"},
