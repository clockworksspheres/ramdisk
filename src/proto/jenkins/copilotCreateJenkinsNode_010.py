...
def generate_keys(key_type="rsa", key_size=2048):
    ...
    #####
    # Changed to OpenSSH below to be compatible with ed25519
...
def main():
    ...
    if args.credential_id:
        if args.use_existing_key:
            private_pem = load_existing_key(args.use_existing_key).strip()
            create_ssh_credential(args.url, args.user, args.password,
                                  args.credential_id, args.ssh_user, private_pem)
        else:
            private_pem, public_pem = generate_keys(args.key_type, args.key_size).strip()
