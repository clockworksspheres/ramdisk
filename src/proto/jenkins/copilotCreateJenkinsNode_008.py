...
def generate_keys(key_type="rsa", key_size=2048):
    ...
    #####
    # TraditionalOpenSSL below needs to be changed to OpenSSH
    # for ed25519 to work.  TraditionalOpenSSL is an older
    # method, and OpenSSH is more flexible and newer.
...
def create_ssh_credential(jenkins_url, user, token, cred_id, ssh_user, private_key):
    ...
    xml_payload = f"""
<com.cloudbees.plugins.credentials.impl.BasicSSHUserPrivateKey plugin="ssh-credentials">
  <scope>GLOBAL</scope>
  <id>{cred_id}</id>
  <description>Created via script</description>
