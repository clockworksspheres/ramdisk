...
class ZoomResizeDemoWindow(QMainWindow):
    def __init__(self):
        ...
        self.setMinimumSize(300, 200)  # Flexible minimum size
        self.setMaximumSize(3840, 2160)  # Support large displays
        self.showNormal()  # Ensure normal window state on macOS
        ...
        try:
            ...
            # Keyboard shortcuts (Command for macOS, Ctrl fallback)
            QShortcut(QKeySequence(Qt.META | Qt.Key_Plus), self, self.zoom_in)
            QShortcut(QKeySequence(Qt.META | Qt.Key_Equal), self, self.zoom_in)
            QShortcut(QKeySequence(Qt.META | Qt.Key_Minus), self, self.zoom_out)
            QShortcut(QKeySequence(Qt.META | Qt.Key_0), self, self.reset_zoom)
            QShortcut(QKeySequence(Qt.CTRL | Qt.Key_Plus), self, self.zoom_in)
            QShortcut(QKeySequence(Qt.CTRL | Qt.Key_Equal), self, self.zoom_in)
            ...
            # Initialize font sizes and table dimensions
            ...
            self.base_row_height = self.table.rowHeight(0) or 30
            self.base_column_width = self.table.columnWidth(0) or 100
    ...
    def resizeEvent(self, event):
        # Debug print for window and widget sizes
        print(f"Window resized to: {event.size().width()}x{event.size().height()}")
        print(f"QTextEdit size: {self.text_edit.size().width()}x{self.text_edit.size().height()}")
        print(f"QWebEngineView size: {self.web_view.size().width()}x{self.web_view.size().height()}")
        print(f"QLabel size: {self.label.size().width()}x{self.label.size().height()}")
        print(f"QGraphicsView size: {self.graphics_view.size().width()}x{self.graphics_view.size().height()}")
        print(f"QTableWidget size: {self.table.size().width()}x{self.table.size().height()}")
        super().resizeEvent(event)

if __name__ == "__main__":
    app = QApplication(sys.argv)
    app.setAttribute(Qt.AA_EnableHighDpiScaling)  # High-DPI support
    app.setAttribute(Qt.AA_UseHighDpiPixmaps)  # Retina display rendering
    try:
