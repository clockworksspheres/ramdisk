...
add_jenkins_test_node.py --url http://localhost:8080 --user admin --token 41b5663... --name debian --method ssh --host <node-ip> --port 22 --credentials-id <jenkins credential id> --executors 2 --labels "debian linux" --remote-fs /home/jenkins/jenkins --description "Debian agent"
...
def create_jnlp_node(server, args):
    ...
    if args.jvm_options:
        launcher_params['jvmOptions'] = args.jvm_options
...
def main():
    ...
    try:
        ...
        if server.node_exists(args.name):
            print(f"Node '{args.name}' already exists → skipping creation.")
            return
        ...
        print("\nSuccess!")
        print(f"→ Node page: {args.url.rstrip('/')}/computer/{args.name}/")
        ...
        if args.method == "jnlp":
            ...
            print(f"Make sure SSH is reachable:  {args.host}:{args.port}")
            print(f"Credential ID '{args.credentials_id}' must be valid.")
