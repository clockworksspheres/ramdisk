  ...
  <disabled>false</disabled>
    ...
    def create_jenkins_pipeline(self, args):
        ...
        try:
            ...
            if args.type == "inline":
                script = self.get_pipeline_script(args)
                config_xml = self.build_inline_config(args, script)
            else:  # scm
                config_xml = self.build_scm_config(args)
...
if __name__ == "__main__":
    create_pipeline = CreateJenkinsPipeline()

    args = parse_arguments()

    create_pipeline.create_jenkins_pipeline(args)
