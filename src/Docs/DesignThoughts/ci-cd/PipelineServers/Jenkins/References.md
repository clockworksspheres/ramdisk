# References

## Jenkins

Youtube: [How To Add Jenkins Controller & Agent Node for Windows | Jenkins Tutorials | Part 6](https://www.youtube.com/watch?v=V2ejGOY_uJI)

Youtube: [How to Create an Agent Node in Jenkins](https://www.youtube.com/watch?v=99DddJiH7lM)

Youtube: [Setup a macOS Agent for Jenkins](https://www.youtube.com/watch?v=DteE1Zf8CIw)

Youtube: [How to Install Jenkins Agent on Windows](https://www.youtube.com/watch?v=N8AQTlHoBKc)


[mac - plugins.jenkins.io](https://plugins.jenkins.io/mac/#plugin-content-macos)

[Jenkins macOS plugin](https://plugins.jenkins.io/mac/)

[How to Build a Python app with PyInstaller using Jenkins](https://dev.to/olalekan_oladiran_d74b7a6/how-to-build-a-python-app-with-pyinstaller-using-jenkins-349k)

[Packaging PySide6 applications for Windows with PyInstaller & InstallForge](https://www.pythonguis.com/tutorials/packaging-pyside6-applications-windows-pyinstaller-installforge/)

[https://www.jenkins.io/doc/tutorials/build-a-python-app-with-pyinstaller/](https://www.jenkins.io/doc/tutorials/build-a-python-app-with-pyinstaller/)

[Pipeline as code with Jenkins](https://www.jenkins.io/solutions/pipeline/)

[Your GitHub-Jenkins Tutorial: Integrating a GitHub Repo With Jenkins](https://www.blazemeter.com/blog/how-to-integrate-your-github-repository-to-your-jenkins-project)

[build-a-python-app-with-pyinstaller.adoc](https://github.com/jenkins-infra/jenkins.io/blob/master/content/doc/tutorials/build-a-python-app-with-pyinstaller.adoc)

[CI/CD for Python Application Using Jenkins and Docker](https://www.fosstechnix.com/ci-cd-for-python-application-using-jenkins/)

[build executable files for mac, windows, linux from your python scripts](https://github.com/LoveBootCaptain/jenkins_test)

Youtube: [Jenkins Tutorial - Create a Pipeline job to build a Python application with PyInstaller](https://www.youtube.com/watch?v=kW_bADC2fFM)

[How Do I Run a Python Script From Jenkins Pipeline?](https://www.youtube.com/watch?v=OB7fGZ32n-s)

[Using Jenkins with a python test](https://www.youtube.com/watch?v=WGvZ_kq5h2o)

[Jenkins pipeline tutoriial](https://www.youtube.com/watch?v=6njM8g5hKuk)

[Automating Jenkins CI server with python - PyCon SG 2015](https://www.youtube.com/watch?v=QyZN1PYagQk)

[Jenkins Tutorial #6 - How to Run Automation Tests in Jenkins](https://www.youtube.com/watch?v=68XLP0ayjPA)

Jenkins Pipeline [Steps](https://www.jenkins.io/doc/pipeline/steps/)
